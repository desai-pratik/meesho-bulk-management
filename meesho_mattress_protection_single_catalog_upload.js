...
const DEFAULTS_FILE = path.join(__dirname, 'single_catalog_mattress_protection_defaults.json');
...
function getDefaults() {
    try {
        ...
        console.error("Error reading single_catalog_mattress_protection_defaults.json:", e.message);
    }
    return {};
}
    ...
    try {
        ...
        for (const group of groups) {
            ...
            try {
                ...
                await clickWithRetry(page, page.getByText('Home Furnishings', { exact: true }).first(), 'Home Furnishings');
                await clickWithRetry(page, page.getByText('Baby & Nursery Bedding', { exact: true }).first(), 'Baby & Nursery Bedding');
                await clickWithRetry(page, page.getByText('Crib Mattress Protection', { exact: true }).first(), 'Crib Mattress Protection');
                ...
                await fillField(page, 'Care Instructions', defaults.careInstructions, true);
                await fillField(page, 'Closure Type', defaults.closureType, true);
                await fillField(page, 'Color', defaults.color, true);
                await fillField(page, 'Filling Material', defaults.fillingMaterial, true);
                await fillField(page, 'Generic Name', defaults.genericName, true);
                await fillField(page, 'Material', defaults.material, true);
                await fillField(page, 'Net Quantity', defaults.netQuantity, true);
                await fillField(page, 'Packaging Breadth', defaults.packagingBreadth, true);
                await fillField(page, 'Packaging Height', defaults.packagingHeight, true);
                await fillField(page, 'Packaging Length', defaults.packagingLength, true);
                await fillField(page, 'Packaging Unit', defaults.packagingUnit, true);
                await fillField(page, 'Pattern', defaults.pattern, true);
                await fillField(page, 'Product Breadth', defaults.productBreadth, true);
                await fillField(page, 'Product Height', defaults.productHeight, true);

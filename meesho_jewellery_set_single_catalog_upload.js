...
function getDefaults() {
    try {
        ...
        console.error("Error reading single_catalog_jewellery_set_defaults.json:", e.message);
    }
    return {};
}
...
async function fillField(page, name, value, isDropdown=false) {
    ...
    try {
        ...
        if (isDropdown) {
            // Find the actual input box inside the dropdown container to avoid clicking info tooltips
            const patterns = [
                `//span[starts-with(normalize-space(.), '${name}')]/parent::div/following-sibling::div//input`,
                `//p[starts-with(normalize-space(.), '${name}')]/parent::div/following-sibling::div//input`,
                `//label[starts-with(normalize-space(.), '${name}')]/following-sibling::div//input`,
                `//*[starts-with(normalize-space(.), '${name}')]/ancestor::div[1]/following-sibling::div//input`
            ];
...
async function fillTableField(page, name, value) {
    ...
    try {
        ...
        if (debugInfo.success) {
            // Use absolute nth locator to survive React destroying elements
            const target = page.locator('input:not([type="checkbox"]):not([type="radio"]):not([type="hidden"]):not([type="file"])').nth(debugInfo.bestInputIndex);
            
            await target.click({ force: true });
            await target.press('Control+A');
            await target.press('Backspace');
            
            // For highly destructive React inputs, pressSequentially re-evaluates the locator every keystroke!
            // We use a massive 200ms delay to give React time to destroy and recreate the input
    ...
    try {
        ...
        for (const group of groups) {
            ...
            try {
                ...
                await clickWithRetry(page, page.getByText('Jewellery Set', { exact: true }).first(), 'Jewellery Set');
                ...
                await fillField(page, 'Included Components', defaults.includedComponents, false);
